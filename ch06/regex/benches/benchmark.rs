...
fn rust_regex(c: &mut Criterion) {
    let mut g = c.benchmark_group("Rust Regex");
    g.measurement_time(Duration::from_secs(12));

    for i in INPUTS {
        g.bench_with_input(i.0, &(i.1, i.2), |b, args| {
            let regex = Regex::new(args.0).unwrap();
            b.iter(|| regex.is_match(args.1))
        });
    }
}
