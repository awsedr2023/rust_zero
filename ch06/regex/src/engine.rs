...
/// ```
/// use my_regex;
/// my_regex::do_matching("abc|(de|cd)+", "decddede", 0, true);
/// ```
///
/// # 引数
///
/// exprに正規表現、lineにマッチ対象とする文字列、offsetにlineの先頭の文字が行頭から何文字目かの数字を与える。
...
///
/// 入力された正規表現にエラーがあったり、内部的な実装エラーがある場合はErrを返す。
pub fn do_matching(expr: &str, line: &str, offset: usize, is_depth: bool) -> Result<bool, DynError> {
    let ast = parser::parse(expr)?;
    let code = codegen::get_code(&ast)?;
    let line = line.chars().collect::<Vec<char>>();
    Ok(evaluator::eval(&code, &line, offset, is_depth)?)
}
